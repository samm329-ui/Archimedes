...
def _eval_easing(t: float, easing: EasingType) -> float:
    t = max(0.0, min(1.0, t))
    if easing == EasingType.LINEAR:            return t
    if easing == EasingType.EASE_OUT_CUBIC:    return 1 - (1 - t) ** 3
    if easing == EasingType.EASE_IN_CUBIC:     return t ** 3
    if easing == EasingType.EASE_IN:           return t ** 2
    if easing == EasingType.EASE_OUT:          return 1 - (1 - t) ** 2
...
def _get_transform(
    ...
    if dur > 0 and start <= frame_index <= start + dur:
        ...
        if best.primitive == MotionPrimitive.TRANSLATE_X:
            ...
            base_area = (element.layout.w_norm * canvas_w) * (element.layout.h_norm * canvas_h)
...
def render_frame(
    ...
    for elem in sorted(template.elements, key=lambda e: e.layer):
        if frame_index < elem.timing.enter_frame or frame_index > elem.timing.exit_frame:
            continue
        ...
        cv2.addWeighted(overlay, tr["opacity"], canvas, 1 - tr["opacity"], 0, canvas)
    ...
    if _HAS_LPIPS:
        try:
            ...
            def _to_tensor(img: np.ndarray):
                ...
                t = torch.from_numpy(img_rgb).float().permute(2, 0, 1).unsqueeze(0) / 127.5 - 1
                return t
            with torch.no_grad():
...
def _text_agreement(rendered: np.ndarray, source: np.ndarray) -> float:
    ...
    """
    size = (160, 90)
    er = cv2.Canny(cv2.cvtColor(cv2.resize(rendered, size), cv2.COLOR_BGR2GRAY), 50, 150)
    ...
    for i in range(len(visible)):
        for j in range(i + 1, len(visible)):
            ...
            if ea.layer == eb.layer:
                ...
                ba = BoundingBox(
                    x=ea.layout.x_norm * 1000, y=ea.layout.y_norm * 1000,
                    w=ea.layout.w_norm * 1000, h=ea.layout.h_norm * 1000,
                )
                bb = BoundingBox(
                    x=eb.layout.x_norm * 1000, y=eb.layout.y_norm * 1000,
                    w=eb.layout.w_norm * 1000, h=eb.layout.h_norm * 1000,
                )
                checks += 1
                if ba.iou(bb) > 0.1:
                    conflicts += 1
...
def compare_frames(
    ...
    passed = (
        ssim >= SSIM_PASS_THRESHOLD and
        pixel >= SIMILARITY_PASS_THRESHOLD
    )
...
def run_render_validation(
    ...
    for attempt in range(MAX_REFINEMENT_ATTEMPTS + 1):
        ...
        if attempt < MAX_REFINEMENT_ATTEMPTS:
            refine_ctx = {
                "mean_ssim": mean_ssim, "mean_lpips": mean_lpips,
                "mean_sim": mean_sim, "pass_rate": pass_rate,
                "frame_results": frame_results, "attempt": attempt,
            }
            template = (
