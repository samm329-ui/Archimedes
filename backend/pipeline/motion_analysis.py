...
def extract_motion_from_track(
    ...
    raw_trajectory = [
        (int(frames[i]), float(cx[i]), float(cy[i])) for i in range(len(frames))
    ]
    ...
    if area_change_pct >= MIN_SCALE_CHANGE_PCT:
        initial_area = float(areas_s[0])
        scale_ratios = areas_s / initial_area
        scale_ratios = np.clip(scale_ratios, 0.1, 10.0)
        hyps = build_motion_hypotheses(
            frames,
            scale_ratios,
        ...
        for h in hyps:
            h.from_value = round(max(0.1, min(10.0, h.from_value)), 4)
            h.to_value = round(max(0.1, min(10.0, h.to_value)), 4)
        all_hypotheses.extend(hyps)
